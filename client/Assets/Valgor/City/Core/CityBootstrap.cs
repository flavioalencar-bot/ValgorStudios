    ...
    public sealed class CityBootstrap : MonoBehaviour, IPlayerCityModule
    {
        ...
        private void Awake()
        {
            ...
            if (Valgor.Core.CityProgressionQa.IsActive)
            {
                ...
                hudQa.Initialize(qa);
                if (Valgor.Core.CityProgressionQa.IsResponsiveUiTest)
                {
                    var responsive = gameObject.GetComponent<Valgor.City.Qa.ResponsiveUiAutoTest>() ??
                ...
                else if (Valgor.Core.DragonPhase2Qa.IsE2ETest)
                {
                    var e2e = gameObject.GetComponent<Valgor.City.Qa.DragonPhase2E2EAutoTest>() ??
                              gameObject.AddComponent<Valgor.City.Qa.DragonPhase2E2EAutoTest>();
                    e2e.Begin(qa, Controller, Dragons);
                }
                else if (Valgor.Core.CityProgressionQa.IsUpgradeUxTest)
                {
