# Build Windows Beta 0.2.2 — NÃO sobrescreve 0.1, 0.2 nem 0.2.1
# Inclui Castelo Tier 1 (FBX real). Fecha Unity Editor antes do batchmode.
param(
...
$log = Join-Path $root "builds\windows\beta-0.2.2-build.log"
$outDir = Join-Path $root "builds\windows\Valgor-Beta-0.2.2"
...
if (Test-Path -LiteralPath $exe) {
  Write-Host "Removendo exe antigo 0.2.2: $exe"
  Remove-Item -LiteralPath $exe -Force
}
if (Test-Path -LiteralPath $dataDir) {
  Write-Host "Removendo Valgor_Data antigo 0.2.2: $dataDir"
  Remove-Item -LiteralPath $dataDir -Recurse -Force
}

$buildStart = Get-Date
Write-Host "Building Valgor Beta 0.2.2 -> $exe (start=$buildStart)"
Write-Host "Log: $log"
Write-Host "Preserva: Valgor-Beta-0.1, 0.2, 0.2.1"
...
$waitDeadline = (Get-Date).AddMinutes(25)
