# Build Windows QA Dragão Fase 4 (montaria + presença mapa).
param(
...
$log = Join-Path $root "builds\windows\qa-dragon-phase4-build.log"
$outDir = Join-Path $root "builds\windows\Valgor-QA-Dragon-Phase4"
$exe = Join-Path $outDir "Valgor.exe"
$dataDir = Join-Path $outDir "Valgor_Data"
$preserve = @(
  (Join-Path $root "builds\windows\Valgor-QA-Dragon-Phase3\Valgor.exe"),
  (Join-Path $root "builds\windows\Valgor-QA-Dragon-Phase2\Valgor.exe")
)
...
$buildStart = Get-Date
Write-Host "Building QA Dragon Phase4 -> $exe"
$code = Invoke-UnityMethod "Valgor.Editor.QaCityProgressionWindowsBuild.BuildDragonPhase4Cli" $log 45
$logOk = (Test-Path $log) -and [bool](Select-String -Path $log -Pattern "Build Successful|Dragon Phase4 Build OK" -Quiet)
