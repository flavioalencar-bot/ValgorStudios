# Build Windows QA Building Upgrade UX (modais Detalhes/Atualizar/Obter + evidências).
param(
...
$log = Join-Path $root "builds\windows\qa-building-upgrade-ux-build.log"
$outDir = Join-Path $root "builds\windows\Valgor-QA-Building-Upgrade-UX"
...
$preserve = @(
  ...
  (Join-Path $root "builds\windows\Valgor-QA-City-Progression-Polished\Valgor.exe"),
...
$buildStart = Get-Date
Write-Host "Building QA Building Upgrade UX -> $exe"
$code = Invoke-UnityMethod "Valgor.Editor.QaCityProgressionWindowsBuild.BuildUpgradeUxCli" $log 45
$logOk = (Test-Path $log) -and [bool](Select-String -Path $log -Pattern "Build Successful|Upgrade UX Build OK" -Quiet)
...
if (-not $SkipAutoTest) {
  $evidence = Join-Path $root "docs\releases\building-upgrade-ux-evidence"
  $report = Join-Path $evidence "auto-test-report.txt"
  if (Test-Path $report) { Remove-Item $report -Force }
  New-Item -ItemType Directory -Force -Path $evidence | Out-Null

  Write-Host "Auto-test building upgrade UX..."
