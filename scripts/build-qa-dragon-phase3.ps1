# Build Windows QA Dragão Fase 3 (habilidades + PvE).
param(
...
$log = Join-Path $root "builds\windows\qa-dragon-phase3-build.log"
$outDir = Join-Path $root "builds\windows\Valgor-QA-Dragon-Phase3"
...
$buildStart = Get-Date
Write-Host "Building QA Dragon Phase3 -> $exe"
$code = Invoke-UnityMethod "Valgor.Editor.QaCityProgressionWindowsBuild.BuildDragonPhase3Cli" $log 45
$logOk = (Test-Path $log) -and [bool](Select-String -Path $log -Pattern "Build Successful|Dragon Phase3 Build OK" -Quiet)
