# Build Windows QA Responsive P1 Fix (1080x640 + 5 resolucoes).
param(
...
$log = Join-Path $root "builds\windows\qa-responsive-p1-build.log"
$outDir = Join-Path $root "builds\windows\Valgor-QA-Responsive-P1-Fix"
$exe = Join-Path $outDir "Valgor.exe"
$dataDir = Join-Path $outDir "Valgor_Data"
$preserve = @(
  (Join-Path $root "builds\windows\Valgor-QA-Building-Construction-Visual\Valgor.exe"),
...
$buildStart = Get-Date
Write-Host "Building QA Responsive P1 -> $exe"
$code = Invoke-UnityMethod "Valgor.Editor.QaCityProgressionWindowsBuild.BuildResponsiveP1Cli" $log 45
$logOk = (Test-Path $log) -and [bool](Select-String -Path $log -Pattern "Build Successful|Responsive P1 Build OK" -Quiet)
...
$evidence = Join-Path $root "docs\releases\ui-responsive-p1-fix-evidence"
New-Item -ItemType Directory -Force -Path $evidence | Out-Null
$report = Join-Path $evidence "auto-test-report.txt"

if (-not $SkipAutoTest) {
  Write-Host "Auto-test responsive UI..."
  Get-Process -Name Valgor -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
  Start-Sleep -Seconds 1
  if (Test-Path $report) { Remove-Item $report -Force }

  $proc = Start-Process -FilePath $exe -ArgumentList @("-responsiveUiTest","-screen-width","1600","-screen-height","900") -WorkingDirectory $outDir -PassThru
  $deadline = (Get-Date).AddMinutes(40)
  while ((Get-Date) -lt $deadline) {
    Start-Sleep -Seconds 5
    if ($proc.HasExited) { break }
    if (Test-Path $report) { Start-Sleep -Seconds 4; break }
  }
