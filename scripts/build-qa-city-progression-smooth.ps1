# Build Windows QA City Progression Smooth (tier swap sem tremor de câmera).
param(
...
$log = Join-Path $root "builds\windows\qa-city-progression-smooth-build.log"
$outDir = Join-Path $root "builds\windows\Valgor-QA-City-Progression-Smooth"
...
$buildStart = Get-Date
Write-Host "Building QA Smooth -> $exe"
$code = Invoke-UnityMethod "Valgor.Editor.QaCityProgressionWindowsBuild.BuildSmoothCli" $log 45
$logOk = (Test-Path $log) -and [bool](Select-String -Path $log -Pattern "Build Successful|Smooth Build OK" -Quiet)
...
if (-not $SkipAutoTest) {
  $evidence = Join-Path $root "docs\releases\tier-swap-smooth-evidence"
  $report = Join-Path $evidence "auto-test-report.txt"
  if (Test-Path $report) { Remove-Item $report -Force }
  New-Item -ItemType Directory -Force -Path $evidence | Out-Null

  Write-Host "Auto-test smooth tier swap..."
