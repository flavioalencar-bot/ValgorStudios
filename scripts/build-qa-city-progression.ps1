# Build Windows QA City Progression (homologação) — não sobrescreve betas 0.1–0.2.4.
param(
...
$log = Join-Path $root "builds\windows\qa-city-progression-build.log"
$outDir = Join-Path $root "builds\windows\Valgor-QA-City-Progression"
...
function Invoke-UnityMethod([string]$method, [string]$logFile, [int]$timeoutMin = 40) {
...
$buildStart = Get-Date
Write-Host "Building QA City Progression -> $exe"
$code = Invoke-UnityMethod "Valgor.Editor.QaCityProgressionWindowsBuild.BuildCli" $log 45
$logOk = (Test-Path $log) -and [bool](Select-String -Path $log -Pattern "Build Successful|QA City Progression Build OK" -Quiet)
...
if (-not $SkipAutoTest) {
  Write-Host "Running auto-test with captures..."
  & (Join-Path $PSScriptRoot "run-city-progression-qa.ps1") -AutoTest
}
